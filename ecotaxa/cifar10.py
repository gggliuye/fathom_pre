...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights', shape=[5, 5, 1, 32],
                                         stddev=1e-4, wd=0.0)
  ...
  with tf.variable_scope('local3') as scope:
    ...
    reshape = tf.reshape(pool3, [-1,2304])

    weights = _variable_with_weight_decay('weights', shape=[2304, 192],
  ...
  with tf.variable_scope('local4') as scope:
    weights = _variable_with_weight_decay('weights', shape=[192, 10],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [10], tf.constant_initializer(0.1))
    local4 = tf.nn.relu(tf.matmul(local3, weights) + biases, name=scope.name)
  
  #local4 = tf.concat([local4,features],1)

  # softmax, i.e. softmax(WX + b)
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [10, 3],
